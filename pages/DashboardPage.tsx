...
import { PricingModel, OrderStatus } from '../types';
import { 
...
const RecentActivities: React.FC = () => {
    ...
    const renderActivity = (activity: any) => {
        ...
        switch (activity.type) {
            case 'quote': return { icon: <QuoteIcon />, title: `Báo giá mới #${activity.id}`, desc: `Đã tạo cho khách hàng ${activity.customer.name}`, time: timeString, bg: 'bg-indigo-100 text-indigo-600' };
            case 'order': return { icon: <OrderIcon />, title: `Đơn hàng #${activity.id}`, desc: `Đã đặt thành công với giá trị ${formatCurrency(activity.totalAmount)}`, time: timeString, bg: 'bg-green-100 text-green-600' };
            case 'customer': return { icon: <CustomerIcon />, title: `Khách hàng mới`, desc: `${activity.name} đã được thêm vào hệ thống`, time: timeString, bg: 'bg-blue-100 text-blue-600' };
                    ...
                    {lowStockItems.map(item => {
                        ...
                        <div key={`${item.type}-${item.id}`} className="group">
                            ...
                            <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-500 ${isCritical ? 'bg-red-500' : 'bg-orange-400'}`} style={{ width: `${percentage}%` }}></div>
                            </div>
                        </div>
                    )})}
                </div>
  ...
  const dashboardData = useMemo(() => {
    ...
    if (timeRange === 'year') {
        ...
        relevantPayments.forEach(p => {
            ...
            monthlyTotals[m].profit += p.amount * 0.35;
        });
    ...
    const categoryColors: Record<string, string> = { 'In tem nhãn': '#3b82f6', 'In Catalogue': '#10b981', 'In Túi giấy': '#f59e0b', 'Sản phẩm khác': '#94a3b8' };
    const distributionData = Array.from(categoryRevenueMap.entries()).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value, color: categoryColors[name] || '#6366f1' })).sort((a, b) => b.value - a.value);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex p-1 bg-gray-100 dark:bg-gray-800 rounded-full border border-gray-200 dark:border-gray-700">
            {['week', 'month', 'year'].map((t) => (
                <TimeRangeButton key={t} label={t === 'week' ? 'Tuần' : t === 'month' ? 'Tháng' : 'Năm'} value={t} activeValue={timeRange} onClick={setTimeRange} />
            ))}
        </div>
      </div>
