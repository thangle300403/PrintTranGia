...
import { RunningCostRule } from '../../types';
...
const RuleModal: React.FC<{
    rule: Partial<RunningCostRule> | null;
    onClose: () => void;
    onSave: (rule: Omit<RunningCostRule, 'id'> | RunningCostRule) => void;
}> = ({ rule, onClose, onSave }) => {
    const [formData, setFormData] = useState<Partial<RunningCostRule>>(
        rule || { threshold: 0, pricePerSheet: 0 }
    );

    const handleChange = (name: keyof RunningCostRule, value: number) => {
        setFormData(prev => ({ ...prev, [name]: value }));
    };

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if ((formData.threshold || 0) <= 0 || (formData.pricePerSheet || 0) < 0) {
            alert('Vui lòng nhập giá trị hợp lệ.');
            return;
        }
        onSave(formData as RunningCostRule);
        onClose();
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-xl w-full max-w-md">
                <h3 className="text-xl font-bold mb-6">{rule?.id ? 'Chỉnh sửa' : 'Thêm'} Bảng giá Công in</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className={labelClass}>Ngưỡng số lượng tờ in</label>
                        <FormattedNumberInput value={formData.threshold || ''} onChange={(val) => handleChange('threshold', val)} className={inputClass} required />
                    </div>
                    <div>
                        <label className={labelClass}>Giá mỗi lượt in (VND)</label>
                        <FormattedNumberInput value={formData.pricePerSheet || ''} onChange={(val) => handleChange('pricePerSheet', val)} className={inputClass} required />
                    </div>
...
const RunningCostsPage: React.FC = () => {
    const { runningCostRules, addRunningCostRule, updateRunningCostRule, deleteRunningCostRule, currentUser, rolePermissions } = useData();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingRule, setEditingRule] = useState<RunningCostRule | null>(null);
    ...
    const handleSave = (ruleData: Omit<RunningCostRule, 'id'> | RunningCostRule) => {
        if ('id' in ruleData && ruleData.id) {
            updateRunningCostRule(ruleData as RunningCostRule);
            setToastMessage('Cập nhật quy tắc thành công.');
        } else {
            addRunningCostRule(ruleData as Omit<RunningCostRule, 'id'>);
    ...
    const confirmDelete = () => {
        if (itemToDelete) {
            deleteRunningCostRule(itemToDelete);
        ...
        <>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h1 className="text-3xl font-bold">Quản lý Bảng giá Công in</h1>
                    <button onClick={() => { setEditingRule(null); setIsModalOpen(true); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold">+ Thêm bậc giá</button>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border overflow-hidden">
                    <table className="min-w-full divide-y">
                        <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-semibold uppercase">Ngưỡng SL tờ in</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold uppercase">Giá / lượt in</th>
                                <th className="px-6 py-3 text-center text-xs font-semibold uppercase">Hành động</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y">
                            {runningCostRules.map(rule => (
                                <tr key={rule.id} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">{rule.pricePerSheet.toLocaleString('vi-VN')} VND</td>
